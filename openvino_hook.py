...
def postprocess(outputs, ctx):
    ...
    boxes[:, :, 1] /= ctx.ratio[0]
    image = ctx.image
    boxes[:, :, 0] = np.clip(boxes[:, :, 0],0,image.shape[1])
    boxes[:, :, 1] = np.clip(boxes[:, :, 1],0,image.shape[0])
...
def resize_image(im,use=512):
    ...
    # limit the max side
    if max(resize_h, resize_w) > use:
        ratio = float(use) / resize_h if resize_h > resize_w else float(use) / resize_w
    else:
        ratio = 1.
