...
def conv2d(input, outputNum, kernel=[3, 3], strides=[1, 1], padding='SAME', bn=False, trainPhase=True, name='conv2d'):
    with tf.name_scope(name) as scope:
        ...
        if bn:
            output = tf.layers.batch_normalization(output, center=False, is_training=trainPhase)
        return output
...
def _text_multibox_layer(inputs,
    ...
    net = inputs
    if normalization > 0:
        net = tf.layers.batch_normalization(net, center=False, is_training=trainPhase)
    num_anchors = 6
    num_classes = 2
    num_loc_pred = 2 * num_anchors * 4

    if is_last:
        loc_pred = tf.layers.conv2d(net, num_loc_pred, [1, 1], activation_fn=None, padding='VALID',
                                    scope='conv_loc')
    else:
        loc_pred = tf.layers.conv2d(net, num_loc_pred, [1, 5], activation_fn=None, padding='SAME',
                                    scope='conv_loc')
    loc_pred = tf.reshape(loc_pred, loc_pred.get_shape().as_list()[:-1] + [2, num_anchors, 4])
    # Class prediction.
    scores_pred = 2 * num_anchors * num_classes
    if is_last:
        sco_pred = tf.layers.conv2d(net, scores_pred, [1, 1], activation_fn=None, padding='VALID',
                                    scope='conv_cls')
    else:
        sco_pred = tf.layers.conv2d(net, scores_pred, [1, 5], activation_fn=None, padding='SAME',
                                    scope='conv_cls')
...
def _box_fn(features, labels, mode, params=None, config=None):
    ...
    glocalisations = labels['glocalisations']
    gscores = labels['glocalisations']
    ...
    with tf.name_scope(name="textbox_layer") as scope:
        ...
        for i, layer in enumerate(layers):
            with tf.variable_scope('{}_box'.format(i)):
                ...
                p, l = _text_multibox_layer(layer, mode == tf.estimator.ModeKeys.TRAIN, is_last=(i == len(layers)),
                                            normalization=norm)
                logits.append(p)
...
def fake_fn(params, is_training):
    ...
    def _data():
        ...
        def _fake(_):
            i = np.zeros([300, 300, 3], dtype=np.float32)
            b = np.zeros([6, 4], dtype=np.float32)
            l = np.zeros([6, 1], dtype=np.int32)
            return i, b, l
        ...
        def _proccess(i, b, l):
            ...
            return {'image': i}, {'glocalisations': glocalisations, 'gscores': gscores}
